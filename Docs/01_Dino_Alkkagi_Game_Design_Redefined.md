...
### 7-1. MVP 이후 체감 임팩트 확장 메모

물리 정확도보다 체감 임팩트가 게임을 결정한다.

MVP에서는 알 충돌, 턴 종료, 승패 판정을 먼저 안정화한다. 이후 여유가 생기면 아래 연출을 모듈처럼 추가해 손맛을 강화한다.

- 알 부딪힐 때 카메라 셰이크: 약하고 짧게 적용한다.
- 알 튕길 때 트레일 이펙트: `LineRenderer` 한 줄 수준으로 시작한다.
- 보드 밖으로 떨어지는 순간 슬로우모션: 약 0.3초만 짧게 적용한다.
- 충돌 시 파티클 이펙트: 먼지, 깃털, 흙 튀김처럼 가볍고 읽기 쉬운 효과를 우선한다.
- 사운드: 알끼리 부딪히는 묵직한 임팩트음과 굴림 소리 루프를 분리한다.
- 그림자: 알이 실제로 있는 위치에 그림자가 자연스럽게 떨어지게 해 3D 게임의 무게감을 만든다.

이 항목들은 현재 MeshCollider 정리 작업 범위가 아니다. Basic Core가 안정된 뒤, Presentation 모듈의 후속 작업으로 다룬다.
